...
class LevelUpEventHandler(AskUserEventHandler):
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[ActionOrHandler]:
        """Dont allow the player to click to exit menu, like normal"""
        return None
    
class InventoryEventHandler(AskUserEventHandler):
    """This handler lets the user select and item"""
    TITLE = "<missing title>"
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        if number_of_items_in_inventory > 0:
            for i, item in enumerate(self.engine.player.inventory.items):
                ...
                is_equipped = self.engine.player.equiment.item_is_equipped(item)
                
                item_string = f"({item.key}) {item.name}"
...
class LookHandler(SelectIndexHandler):
    """Let player look around using the keyboard"""
